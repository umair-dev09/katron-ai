// API proxy route for checking order status
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[Check Order Status] Checking status for order:", giftCardOrderId)

    const response = await fetch(
      `${EXTERNAL_API_BASE_URL}/api/giftCards/checkForPaymentAndGiftCardStatus?giftCardOrderId=${giftCardOrderId}`,
      {
        method: "POST",
        headers: {

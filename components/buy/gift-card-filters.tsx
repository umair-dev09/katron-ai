...
interface GiftCardFiltersProps {
  ...
  setSelectedFilters: (filters: { stores: string[]; categories: string[] }) => void
  availableCategories?: string[]
  availableStores?: string[]
}

// Fallback values if API doesn't return categories/stores
const DEFAULT_CATEGORIES = ["Baby & Kids", "Books", "Education", "Electronics", "Fashion", "Food & Dining", "Gaming", "Travel"]
...
export default function GiftCardFilters({ 
  ...
  // Filter out non-string values and apply search
  const filteredStores = availableStores
    .filter((store): store is string => typeof store === "string" && store.length > 0)
  ...
  // Only show store filter if there are stores available
  const showStoreFilter = filteredStores.length > 0

  return (
    <div className="space-y-6">
      {/* Store Filter - Only show if stores available */}
      {showStoreFilter && (
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {filteredStores.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground text-center py-2">No brands found</p>
                )}
              </div>
            </div>
          )}
      ...
      {(selectedFilters.stores.length > 0 || selectedFilters.categories.length > 0) && (
        <div className="text-sm text-muted-foreground">
          {selectedFilters.stores.length + selectedFilters.categories.length} filter(s) applied
        </div>
      )}
      ...
      <Button
        ...
        disabled={selectedFilters.stores.length === 0 && selectedFilters.categories.length === 0}
        className="w-full bg-gray-50 dark:bg-gray-900/20 hover:bg-gray-100 dark:hover:bg-gray-900/30 text-foreground border border-gray-200 dark:border-gray-800 rounded-lg disabled:opacity-50"
      >
        Reset Filters
      </Button>
    </div>
